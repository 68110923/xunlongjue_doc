...
- 🔧 **振幅闸微调** — 12.0%→12.5%，避免边界误杀（如新金路12.1%被卡）。
- ⚡ **盘中刷新重构** — 拉行情→读历史K线(单次DB)→内存算全指标→一次性写库。从46s降到11s。
- 🐛 **选好票VR补算** — `_full_indicators` 末尾增加量比计算，修复盘中VR全NULL问题。选好票增加VR≥1.2前置过滤，排除缩量弱势票。
- ⚖️ **全局评分权重重校准** — 9维度评分全面调参：核心维度（Gap/封板时间）微降峰值，弱势维度（封板强度/KDJ/首板Gap）拉高权重增加颗粒度，次要维度（市值/量比/换手率）适度压缩。总控±100内。
- 📐 **量比重构** — 硬闸VR下限上调，评分重写：缩量区线性扣分，温和放量平台满分，放量偏大线性递减。整体跨度扩大，提升量比维度权重。
