...
<details>
<summary><b>怎么根据自己的思路二次开发？</b></summary>
<p>寻龙诀不是封闭成品，源码全交付，底层数据库、算法函数都是开放的，你可以按自己的思路扩展：</p>
<p>🔧 <strong>依赖寻龙诀，写你自己的项目</strong> —— 在寻龙诀项目外新建一个项目，直接引用它的数据库和算法函数，写自己的选股条件、指标组合、分析工作流。外建项目不动寻龙诀本身，<strong>系统更新（策略迭代、代码修复）照常自动同步，不受影响</strong>，你写的东西也都在，两边互不干扰。</p>
<p>🧠 <strong>方法论落地成代码</strong> —— 把你的交易思路翻译成代码，跑历史回测验证；或把数据查出来喂给大模型，按你的方法论自动分析。</p>
<p>🤖 <strong>不会写代码也能改</strong> —— 思路说给 AI，让它帮你写代码、做回测、调参数，AI 就是你的程序员。</p>
<p>📦 <strong>可直接依赖的模块</strong>（scripts/ 脚本 + references/ 文档）：</p>
<p>▸ <code>hermes_stock_lib.py</code> — 核心算法库：评分、硬闸、连板判定，二次开发最常依赖<br>
▸ <code>data_fetcher.py</code> — 数据工厂：全市场日线、数据自动治理<br>
▸ <code>backtest.py</code> — 回测框架：验证自己的策略<br>
▸ <code>market_signal.py</code> / <code>deep_analysis.py</code> / <code>stock_screener.py</code> — 预测、深度分析、选好票现成逻辑<br>
▸ <code>morning_push.py</code> / <code>afternoon_review.py</code> — 早盘推送、收盘复盘流程参考<br>
▸ <code>strategy.md</code> — 策略文档：全部判定逻辑的参数说明<br>
▸ <code>mingri-guancha-workflow.md</code> — 明日观察工作流<br>
▸ <code>xunlongjue-mind.md</code> — 心法：AI 输出风格与思考框架</p>
</details>

---

<p align="center">
  <sub>⚠️ 行情数据来源：同花顺 / 新浪财经 / 东方财富 / baostock / akshare | 仅供研究学习，不构成投资建议 | 市场有风险，投资需谨慎</sub>
</p>
...
### V20260916 (2026-09-16)
- 📦 **数据库交付通道上线** — 客户拿库改为预签名链接一键下载，无需再靠网盘/分卷传输；一条命令完成上传+出链接+生成可转发的交付文案，部署前拿到数据库即可直接开跑。
- 📄 **部署文档重构** — 部署步骤按「四步前置 → 克隆 → 导入 → 注册 → 定时任务」重排，命令可直接复制执行，首次部署路径更清晰。
- 🔗 **下载地址集中配置** — 交付链接地址不再写死在代码里，统一从服务端配置读取，换服务器或换下载入口只需改一处，交付脚本自动跟随。
- 🔒 **交付链接安全收紧** — 链接有效期默认收紧至 12 小时；链接凭据不再明文长期留档，只记录可追溯标识，降低泄露面。

### V20260913 (2026-09-13)
- 🏷️ **数据来源标注补全** — 页脚与说明文档同步补全实际接入的全部行情数据源（新增东方财富、akshare 等），此前遗漏多个在用来源；仅修正声明口径，不影响任何取数逻辑。

### V20260908 (2026-09-08)
- 🔧 **回测默认含最高分行** — 候选池最高分行默认展示，推送与漏选表现一键对比；仅看推送口径加 `--push-only` 参数。汇总均值随模式切换，命中统计始终按推送行计算。
